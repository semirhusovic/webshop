                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center space-x-4 text-sm">
                                ...
                                <form id="delete{{$manufacturer->id}}" method="post" action="{{route('manufacturer.destroy',$manufacturer->id)}}">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" onclick="confirmDelete(event,{{$manufacturer->id.','. "'".$manufacturer->manufacturerName."'"}})" class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray" aria-label="Delete">
                                        <svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20">
