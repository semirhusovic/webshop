...
{{--                        </td>--}}
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center space-x-4 text-sm">
                                ...
                                <form id="delete{{$country->id}}" method="post" action="{{route('country.destroy',$country->id)}}">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" onclick="confirmDelete(event,{{$country->id.','. "'".$country->countryName."'"}})" class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray" aria-label="Delete">
                                        <svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20">
        ...
        <script>
            function confirmDelete(e,id,name){
                ...
                Swal.fire({
                    title: 'Are you sure?',
                    text: "You won't be able to restore " + name + ' country',
                    icon: 'warning',
